...
| **Heat-sink challenge** | Dissipate a given power below a maximum temperature, with less material | *T*, heat flux, *T*<sub>max</sub>, *R*<sub>θ</sub>, mass, fin efficiency | energy balance, 1-D fin theory, a bare-plate limiting case, and `mock.heat2d` against `dolfinx.heat2d` — the only cross-adapter check in the lab, and it is free | **specified**: [`exercises/heat-sink.md`](exercises/heat-sink.md). The only exercise blocked on nothing upstream, and the first that needs no solver of its own. Its one modelling decision is that *h* is a function of channel spacing: held constant, the solve says more fins are always better, which is the opposite of the lesson |
